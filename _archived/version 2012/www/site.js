...
function menuArrows(row) {
	...
	$('.menu-row').each(function() {
		...
		if (scrollItemsLeftPos < scrollBoxLeftPos) {
			$(this).parent().find('.menu-leftarrow').fadeIn(200);
		}
		if (scrollItemsLeftPos >= scrollBoxLeftPos) {
			$(this).parent().find('.menu-leftarrow').fadeOut(200);
		}
		if (scrollItemsRightPos > scrollBoxRightPos) {
			$(this).parent().find('.menu-rightarrow').fadeIn(200);
		}
		if (scrollItemsRightPos <= scrollBoxRightPos) {
			$(this).parent().find('.menu-rightarrow').fadeOut(200);
		}
	});
}
...
$(document).ready(function() {
	...
	// CUFON
	Cufon.replace('h1', { fontFamily: 'Tungsten' });
	Cufon.replace('h2', { fontFamily: 'Tungsten' });
	Cufon.replace('h3', { fontFamily: 'Tungsten' });
	Cufon.replace('h4', { fontFamily: 'Tungsten' });
	Cufon.replace('h5', { fontFamily: 'Tungsten' });
	Cufon.replace('#navbox li', { fontFamily: 'Tungsten' });
	...
	// DOES NOT WORK - CHECK COMPATIBILITY WITH JQUERY 1.4.4
	//$(function()
	//{
	//	$('#port-info').jScrollPane();
	//	$('#port-dropdown').jScrollPane();
	//});
	var winheight = $(window).height() - 20;
	$('#port-content').css('height', winheight);
	$(window).resize(function() {
		var winheight = $(window).height() - 20;
		$('#port-content:visible').css('height', winheight);
	});
